...
public class TrieBuilderModule extends ModuleImpl {
	...
	private static final String ID_INPUT = "segments";
	private static final String ID_OUTPUT = "trie";
	
	// Local variables
	private String inputDelimiter = "";
	private int maxDepth = -1;
	private boolean omitRedundantInformation;
	private boolean compactTrie;
	//private int maxThreads;

	public TrieBuilderModule(CallbackReceiver callbackReceiver,
		...
		this.setDescription("Constructs a trie (prefix tree, radix tree) from segmented text input.");
		...
		this.getPropertyDescriptions().put(PROPERTYKEY_MAXDEPTH, "Maximum depth for the resulting trie; set to -1 for no constraint.");
		...
		this.getPropertyDescriptions().put(PROPERTYKEY_STRUCTURE, "Type of trie to output; possible values are 'compact' (Patricia Trie) and 'atomic'.");
		...
		while (inputScanner.hasNext()){
			...
			while (leaves.hasNext()){
				...
				if (childNode == null){
					...
					if (compactTrie && !node.equals(rootNode) && node.getChildNodes().size() == 0  && !nextLeafList.contains(node)){
					...
					// Merge with parent or construct separate child node
					if (compactTrie && !node.equals(rootNode)
